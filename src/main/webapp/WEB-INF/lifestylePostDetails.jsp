...
<body>
	<div class="header">
		...
		<div class="navbar">
			<a href="/studygroups">Study Groups</a> <a href="/dashboard/jobposts">Jobs</a>
			<a href="/dashboard/interviewposts">Interview Prep</a> <a
				href="/dashboard/lifestyleposts">LifeStyle</a> <a
				href="/dashboard/successstories">Success Stories</a> <a
	...
	<div class="section body-main">
		<div class="text-center">
			<h1 class="lifestylepost-heading">
				<c:out value="${oneLifestylePost.title }" />
			</h1>
		</div>
		<div class="d-flex align-items-top details-body">
			<div class="body-left">
				<img
					src="<c:url value='${oneLifestylePost.photosImagePath}'></c:url>"
					alt="image" class="details_image" />
				<div class="d-flex mt-3">
					<h6 class="me-1">
						Posted by
						<c:out value="${oneLifestylePost.lifestylePostCreator.userName }" />
					</h6>
					<c:choose>
						<c:when test="${oneLifestylePost.updatedAt != null }">
							<h6 class="text-secondary fw-light fst-italic">
								(
								<fmt:formatDate pattern="MMM dd, yy hh:mm a" type="both"
									value="${oneLifestylePost.updatedAt }" />
								)
							</h6>
						</c:when>
						<c:otherwise>
							<h6 class="text-secondary fw-light fst-italic">
								(
								<fmt:formatDate pattern="MMM dd, yy hh:mm a" type="both"
									value="${oneLifestylePost.createdAt }" />
								)
							</h6>
						</c:otherwise>
					</c:choose>
			...
			<div class="body-right">
				<h4>
					<c:out value="${oneLifestylePost.headline }" />
				</h4>
				<p class="mt-3 post-description">
					<c:out value="${oneLifestylePost.description }" />
				</p>
			</div>
		</div>
		<hr />
		<div class="d-flex justify-content-end">
			<c:if test="${currentUser.id.equals(oneLifestylePost.lifestylePostCreator.id) }">
				<div class="d-flex">
					<a class="btn newpost-button button-post"
						href="/lifestyleposts/${oneLifestylePost.id }/edit">Edit</a>
					<form action="/lifestyleposts/${oneLifestylePost.id }/delete"
						method="POST">
			...
			<a class="btn btn-warning button-dashboard"
				href="/dashboard/lifestyleposts">Lifestyle Post Dashboard</a>
		</div>
		<form:form action="/lifestyleposts/${oneLifestylePost.id }/comment"
			method="POST" modelAttribute="newComment" class="form mt-3">
			<input type="hidden" name="${_csrf.parameterName}"
				value="${_csrf.token}" />
			<p>
			...
			<div class="d-flex justify-content-end">
				<button class="btn newpost-button" type="submit">Add
					Comment</button>
			</div>
		</form:form>
		<c:choose>
